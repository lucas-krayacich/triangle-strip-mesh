...
#choose next triangle iteratively until the strip can no longer continue
def chooseNext(triangle):

    triangle.nextTri = findNewTri(triangle.adjTris)

    while triangle.nextTri is not None:
...
#uses a for loop to find a triangle within the given list that will be the most optimal starting point or next triangle
#The optimal choice is the triangle with the least open adjacent triangles and the least open adjacent-adjacent triangles
def findNewTri(triangles: list [Triangle]):
    ...
    for opt in triangles:
        adjcount = 0
        if opt.nextTri is None and opt.prevTri is None:
            numAdj = len(opt.adjTris)

            for adj in opt.adjTris:
                if adj.nextTri is None and adj.prevTri is None:
                    adjcount += 1

            if numAdj <= baseAdj and adjcount <= baseAdjAdj:
                baseAdj = numAdj
                baseAdjAdj = adjcount
                chosenOpt = opt
...
def buildTristrips( triangles ):
    ...
    while 1:
        #Set beginning of new strip to corner (ideally)
        currentTri = findNewTri(triangles)

        #no triangles left to fill
        if currentTri is None:
            break

        #iteratively choose the next triangle (iteration happens within the function itself)
        chooseNext(currentTri)
        #increase count at the end of every strip
        count += 1
